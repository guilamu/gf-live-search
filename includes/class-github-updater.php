<?php
/**
 * GitHub Auto-Updater for GF Live Search.
 *
...
/**
 * Class GF_Live_Search_GitHub_Updater
 */
class GF_Live_Search_GitHub_Updater {
    ...
    private const GITHUB_REPO        = 'gf-live-search';
    private const PLUGIN_FILE        = 'gf-live-search/gf-live-search.php';
    private const PLUGIN_SLUG        = 'gf-live-search';
    private const PLUGIN_NAME        = 'GF Live Search';
    private const PLUGIN_DESCRIPTION = 'Adds live filtering to the Gravity Forms forms list. As you type in the search box, forms are instantly filtered without a page reload.';
    ...
    private const GITHUB_TOKEN       = '';    // leave empty for public repos
    ...
    /**
     * Get the download URL, preferring custom .zip assets over GitHub's zipball.
     *
    ...
    public static function plugin_info_css(): void {
        ...
        if (
            'plugin-information' !== sanitize_text_field( wp_unslash( $_GET['tab'] ) ) ||
    ...
    /**
     * Convert Markdown to HTML using Parsedown (if available) or a basic fallback.
     *
    ...
    private static function markdown_to_html( string $markdown ): string {
        ...
        if ( class_exists( 'Parsedown' ) ) {
            ...
            // Basic fallback: paragraphs and line breaks only.
            $html = '<p>' . implode( '</p><p>', array_filter( array_map( 'trim', explode( "\n\n", esc_html( $markdown ) ) ) ) ) . '</p>';
        }
...
GF_Live_Search_GitHub_Updater::init();
